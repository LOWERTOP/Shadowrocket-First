...
> 
> > 本模块由 [iab0x00](https://github.com/iab0x00) 维护，支持去除该软件的各类广告并提供画中画功能，同时支持 Youtube Music 去广告。本模块支持 **编辑参数**，安装后可点击模块进入参数编辑页面，可选屏蔽上传按钮、Shorts、选段按钮，以及设置语言等。需要注意，Apple TV 版 YouTube 不支持解密，不可使用类似模块。其中第二个版本是为降低内存消耗而做的备用版本，同样包含编辑参数功能
> >   
