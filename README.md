...
> `  小火箭配色文件使用说明  `
> 
> * 选择所需的配色，在链接代码 ` shadowrocket://color?* ` 右侧点击“ ❐ ”符号复制链接，在软件配置页面点击 “ ✚ ” 号，粘贴链接并应用
> 
...
> ✨ `  用前必读  ` ✨
> 
> 因苹果系统限制，Shadowrocket APP 部分细节图标不可自定义色彩，且会根据系统深色/浅色模式改变，使用小火箭自定义配色时可能会因为此特性导致局部细节图标可视度存在问题，建议在使用 `浅底色` 时搭配系统浅色模式，使用 `暗底色` 时搭配系统深色模式效果更佳，望知悉。另外，配色文件以实际效果为准，效果展示仅作示意，实际更新或存在调整  [反馈途径](https://t.me/ShadowrocketApp)
